...
@interface TerminalStatusTest : BlockChypTest



@end

@implementation TerminalStatusTest
...
- (void)testTerminalStatus{
  ...
  XCTestExpectation *expectation = [self expectationWithDescription:@"TerminalStatus Test"];
  ...
  [client terminalStatusWithRequest:request handler:^(NSDictionary *request, NSDictionary *response, NSError *error) {
    ...
    XCTAssertTrue([[response objectForKey:@"idle"]boolValue]);
