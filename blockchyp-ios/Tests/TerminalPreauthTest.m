...
@interface TerminalPreauthTest : BlockChypTest



@end

@implementation TerminalPreauthTest
...
- (void)setUp {
  ...
  [self testDelayWith:client testName:@"TerminalPreauthTest"];
...
- (void)testTerminalPreauth{
  ...
  XCTestExpectation *expectation = [self expectationWithDescription:@"TerminalPreauth Test"];
  ...
  [client preauthWithRequest:request handler:^(NSDictionary *request, NSDictionary *response, NSError *error) {
    ...
    XCTAssertEqualObjects(@"15.15", (NSString *)[response objectForKey:@"authorizedAmount"]);
