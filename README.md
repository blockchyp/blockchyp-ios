...
int main (int argc, const char * argv[])
{
  ...
  NSMutableDictionary *request = [[NSMutableDictionary alloc] init];
    [client surchargeReviewWithRequest:request handler:^(NSDictionary *request, NSDictionary *response, NSError *error) {
...
class ExampleClass {
  ...
  func example() {
    ...
    var request: [String:Any] = [:]
      client.surchargeReview(withRequest: request, handler: { (request, response, error) in
...
int main (int argc, const char * argv[])
{
  ...
  NSMutableDictionary *request = [[NSMutableDictionary alloc] init];
    [client transientKeyWithRequest:request handler:^(NSDictionary *request, NSDictionary *response, NSError *error) {
      NSNumber *success = [response objectForKey:@"success"];
    if (success.boolValue) {
      NSLog(@"Success");
    }
    NSLog(@"%@: %@", @"apiKey", [response objectForKey:@"apiKey"])
    NSLog(@"%@: %@", @"bearerToken", [response objectForKey:@"bearerToken"])
    NSLog(@"%@: %@", @"signingKey", [response objectForKey:@"signingKey"])
  }];
  [pool drain];
  return 0;
}
...
class ExampleClass {
  ...
  func example() {
    ...
    var request: [String:Any] = [:]
      client.transientKey(withRequest: request, handler: { (request, response, error) in
        let approved = response["success"] as? Bool
      if (approved.unsafelyUnwrapped) {
        NSLog("Success")
      }
      NSLog("apiKey" + ": " + (response["apiKey"] as? String).unsafelyUnwrapped)
      NSLog("bearerToken" + ": " + (response["bearerToken"] as? String).unsafelyUnwrapped)
      NSLog("signingKey" + ": " + (response["signingKey"] as? String).unsafelyUnwrapped)
    })
  }
